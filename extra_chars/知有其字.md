...
- Every element on the [periodic table](化学.md) gets its own Chinese character.

- Each Chinese province, etc., is assigned a [province abbreviations](省份简称.md).

- There are characters which are composed on [repeated components](重叠字.md), which are often used decoratively.
...
- Characters relating to [mammals](./哺乳动物.md) [first draft], [plants and flowers](./花卉.md) [pre-first draft], [musical instruments](./乐器.md) [first draft].  

- Characters in names: [historical](./历史人名.md) [first draft], [given](./人名.md) [first draft], [family](./姓氏.md) [pre-first draft].

- There are some rare characters that you might see [in the wild](可遇字.md).
